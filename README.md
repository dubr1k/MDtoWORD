...
This installs `requirements-core.txt` (the shared conversion dependencies)
plus the `mcp` SDK — not PyQt6, so the server stays runnable headless.
...
claude mcp add mdtoword --scope user \
  --env PYTHONPATH=/path/to/MDtoWord \
  -- /path/to/MDtoWord/.venv/bin/python -m mdtoword.mcp_server
```

`PYTHONPATH` is required because this project runs from a checkout rather
than an installed package — without it, `mdtoword.mcp_server` is not
importable once Claude Code starts the server in your own project's
...
The suite currently holds **200 tests**. `QT_QPA_PLATFORM=offscreen` lets the interface tests run without a display.
...
Это ставит `requirements-core.txt` (общие зависимости конвертации) и SDK
`mcp` — без PyQt6, так что сервер остаётся запускаемым без дисплея.
...
Переменная `PYTHONPATH` нужна: проект запускается из чекаута, а не установлен
как пакет, и без неё `mdtoword.mcp_server` не импортируется, как только Claude
Code запускает сервер в рабочей директории вашего проекта, а не в этом
репозитории.
...
Сейчас в наборе **200 тестов**. Переменная `QT_QPA_PLATFORM=offscreen` нужна, чтобы тесты интерфейса работали без экрана.
